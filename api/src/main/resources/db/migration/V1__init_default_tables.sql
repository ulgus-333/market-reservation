...
CREATE TABLE user.role (
    idx int unsigned auto_increment COMMENT '식별번호' primary key,
...
CREATE TABLE user.user (
    idx int unsigned auto_increment COMMENT '식별번호' primary key,
    role_idx int unsigned not null COMMENT '권한 식별번호',
...
CREATE TABLE user.user_identify (
    user_idx int unsigned not null COMMENT '유저 식별번호' primary key,
...
CREATE TABLE user.admin_identify (
    user_idx int unsigned not null COMMENT '유저 식별번호' primary key,
    market_idx int unsigned not null COMMENT '상점 식벌번호',
    department_idx int unsigned null COMMENT '부서 식별번호',
...
CREATE TABLE user.console_identify (
    user_idx int unsigned not null COMMENT '유저 식별번호' primary key,
    department_idx int not null COMMENT '부서 식별번호',
    position varchar(20) not null COMMENT '직급 명칭',
    name varchar(50) not null COMMENT '이름(양방향 암호화)',
    email varchar(50) not null COMMENT '이메일',
    reg_datetime datetime not null default current_timestamp COMMENT '등록일시',
    mod_datetime datetime null COMMENT '수정일시'
...
CREATE TABLE user.department (
    idx int unsigned auto_increment COMMENT '부서 식별번호' primary key,
    market_idx int unsigned not null COMMENT '관리 상점 인덱스 번호 / 0: 백오피스',
    name varchar(20) not null COMMENt '부서명',
    menu_authority_idx int unsigned not null COMMENT '메뉴권한 식별번호',
    reg_datetime datetime not null default current_timestamp COMMENT '등록일시',
    reg_idx int unsigned not null default 0 COMMENT '등록자 식별번호 / 0: 시스템',
    mod_datetime datetime null COMMENT '수정일시',
    mod_idx int unsigned null COMMENT '등록자 식별번호 / 0: 시스템'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='부서정보';

CREATE TABLE common.menu (
    idx int unsigned auto_increment COMMENT '메뉴 식별번호' primary key,
...
CREATE TABLE user.menu_authority (
    idx int unsigned auto_increment COMMENT '메뉴권한 식별번호' primary key,
    market_idx int unsigned not null COMMENT '관리 상점 식별번호 / 0: 백오피스',
    name varchar(50) not null COMMENT '권한 명칭',
    reg_datetime datetime not null default current_timestamp COMMENT '등록일시',
    reg_idx int not null default 0 COMMENT '등록자 식별번호 / 0: 시스템',
    mod_datetime datetime COMMENT '수정일시',
    mod_idx int COMMENT '수정자 식별번호 / 0: 시스템'
...
CREATE TABLE user.permitted_menu_authority (
    idx int unsigned auto_increment COMMENT '접근 가능 메뉴 식별번호' primary key,
    menu_authority_idx int unsigned not null COMMENT '메뉴 권한 식별번호',
    menu_idx int unsigned not null COMMENT '접근 가능 메뉴 식별번호',
    reg_datetime datetime not null default current_timestamp COMMENT '등록일시',
    reg_idx int unsigned not null default 0 COMMENT '등록자 식별번호 / 0: 시스템',
    mod_datetime datetime null COMMENT '수정일시',
    mod_idx int unsigned COMMENT '수정자 식별번호 / 0: 시스템'
...
CREATE TABLE market.market (
    idx int unsigned auto_increment COMMENT '상점 식별번호' primary key,
